...
describe('logoutUser — POST /auth/logout', () => {
  it('sends POST and returns undefined on 204', async () => {
...
describe('deleteMyAccount — DELETE /users/me', () => {
  it('sends confirm:true body and returns undefined on 204', async () => {
    mockFetch([{ status: 204, body: null }]);
  ...
  it('throws ApiError(400) when backend returns DeletionNotConfirmedError', async () => {
    mockFetch([{ status: 400, body: { detail: 'Deletion not confirmed' } }]);

    await expect(deleteMyAccount()).rejects.toMatchObject({ status: 400 });
  });
});
